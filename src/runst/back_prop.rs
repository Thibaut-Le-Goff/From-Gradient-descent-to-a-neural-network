...
pub fn grad_descent(net: &Network, observed_values: &Vec<Vec<f32>>, weights: &Vec<Vec<f32>>, bias: &Vec<Vec<f32>>) {
    ...
    // The variable who will contains the value to the next value of 
    ...
    let mut slope_intercept: [f32; 2] = [weight[0], bias[0]];
    // pour 0:
    ...
    // for each number of try
        ...
        // weights (weights_bias[0]) and for each bias afterward (weights_bias[1]).
            if slope_intercept_trouve[y] == false {
                ...
                for j in 0..= datas.observed_values.len() - 1 {
                    // for each pair of datas (given and observed), samples:
                    predicted_height = (slope_intercept[0] * /*what give last layer */[j]) + slope_intercept[1];
                    ...
                    if y == 0 {
                        ...
                        derivative_square_residual = (-power_dif * /*what give last layer */[j]) * (datas.observed_values[j] - predicted_height);
                    ...
                    if y == 1 {
                        ...
                        derivative_square_residual = -power_dif * (datas.observed_values[j] - predicted_height);
                        sum_derivative_square_residual = derivative_square_residual + sum_derivative_square_residual;
                    }
                }
                ...
                // determination de la prochaine valeur du coéficient directeur ou de l'intercept
